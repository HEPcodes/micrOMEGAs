...
static int alg2_red_orth_11(List *lm, List l1, Term prop1, Term prop2)
	...
	{
	  Atom a;
	  a=CompoundArgN(CompoundArgN(matr,IntegerValue(CompoundArgN(prop1,3))),
				IntegerValue(CompoundArgN(prop1,4)));
	  if(a!=el_name[0])
	  {
		if(GetAtomProperty(a,A_ANTI)==el_name[0]) inv1=1;
		else puts("Internal error (ro11i1)");
	  }
	  a=CompoundArgN(CompoundArgN(matr,IntegerValue(CompoundArgN(prop2,3))),
				IntegerValue(CompoundArgN(prop2,4)));
	   if(a!=el_name[1])
	  {
