...
*  Diagonalizing the scharm mass-matrices
     ...
     .                                                          /2d0
      ...
      RSC(1,1)=dcos(thetaSC)
      RSC(1,2)=-dsin(thetaSC)
      RSC(2,1)=dsin(thetaSC)
      RSC(2,2)=dcos(thetaSC)
...
*          - New physics contribution to epsK [5] Eqs.(7.26)-(7.27)
     ...
     .           *dsqrt(2d0)*dsin(43.52d0/180d0*PI)    ! Corrections from angle =/= Pi/4
...
*      (2sigma, added quadratically)
     ...
     .           *2d0*dsin(43.52d0/180d0*PI)/3.48d-15)**2*epsKMax
      epsKmin=epsKMax
...
*      Second: Uncertainty from matching: 30% on each BSM contribution
     ...
     .      *MK0*FK0**2*aux/3.48d-15*dsqrt(2d0)*dsin(43.52d0/180d0*PI)    ! Corrections from angle =/= Pi/4
...
*          - Long-distance effects [7]
      ...
      epsKSM=epsKSM+dsin(43.52d0/180d0*PI)*(-1.63d-4)
