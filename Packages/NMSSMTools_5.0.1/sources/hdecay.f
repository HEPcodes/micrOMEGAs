...
*      HILLLR(i)       coupling ai,~e_L,~e_R (i=1,2, 3rd family only)
      ...
      DOUBLE COMPLEX CXTSM,CXBSM,CXCSM,CXLSM,CXSSM,CXMSM
...
*    to 2 photons as compared to the SM coupling
       ...
       CG(I)= CG(I)/CDABS(4d0/3d0*(CXT+CXC)+(CXB+CXS)/3d0+CXL+CXM+CXW)
...
*  Effective coupling to 2 gluons/2 photons
       ...
       CTS= 4d0*(MSTRANGE/MA)**2*DCMPLX(1d0,-EPS)
       CTL= 4d0*(MTAU/MA)**2*DCMPLX(1d0,-EPS)
       CTM= 4d0*(MMUON/MA)**2*DCMPLX(1d0,-EPS)
...
*   them by the SM couplings in order to obtain the reduced couplings
     ...
     .  + CB(I+3)*CXB/3d0 + CD(I+3)*(CXS/3d0+CXL+CXM)
...
*    (CJ does not contain strange quark loops)
       ...
       CXSSM= 2d0*CTS*(1d0+(1d0-CTS)*CF(CTS))
