...
*   Check for errors
 ...
 29   ERR=0
      ...
      IF(MHDMIN.EQ.1d99 .AND. MHDMAX.NE.1d99)THEN
       WRITE(0,1)"MHD_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(MHUMIN.EQ.1d99 .AND. MHUMAX.NE.1d99)THEN
       WRITE(0,1)"MHU_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      IF(MSMIN.EQ.1d99 .AND. MSMAX.NE.1d99)THEN
       WRITE(0,1)"MS_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
...
*   Warnings
      ...
      IF(M3HMIN.EQ.M3HMAX .AND. NM3H.GT.1)THEN
       WRITE(0,1)"WARNING M3H_min=M3H_max => NM3H=1"
       NM3H=1
      ENDIF
      ...
      IF(MAFLAG.EQ.-2)THEN
       ...
       IF(MSMIN.EQ.MSMAX .AND. NMS.GT.1)THEN
        WRITE(0,1)"WARNING MS_min=MS_max => NMS=1"
        NMS=1
        N2=NMS
       ENDIF
       IF(XIFMIN.NE.XIFMAX .AND. NXIF.EQ.1)THEN
       ...
       IF(MSMIN.NE.MSMAX .AND. NMS.EQ.1)THEN
        WRITE(0,10)"WARNING NMS=1 => MS_max=MS_min=",MSMIN
        MSMAX=MSMIN
       ENDIF
      ENDIF
      ...
      ELSE
       IF(MHDMIN.EQ.MHDMAX .AND. NMHD.GT.1)THEN
        WRITE(0,1)"WARNING MHD_min=MHD_max => NMHD=1"
        NMHD=1
        N3=NMHD
       ENDIF
       IF(MHUMIN.EQ.MHUMAX .AND. NMHU.GT.1)THEN
        WRITE(0,1)"WARNING MHU_min=MHU_max => NMHU=1"
        NMHU=1
        N4=NMHU
       ENDIF
       IF(MHDMIN.NE.MHDMAX .AND. NMHD.EQ.1)THEN
        WRITE(0,10)"WARNING NMHD=1 => MHD_max=MHD_min=",MHDMIN
        MHDMAX=MHDMIN
       ENDIF
       IF(MHUMIN.NE.MHUMAX .AND. NMHU.EQ.1)THEN
        WRITE(0,10)"WARNING NMHU=1 => MHU_max=MHU_min=",MHUMIN
        MHUMAX=MHUMIN
       ENDIF
      ENDIF
     ...
     . MSFLAG,AKFLAG,ALFLAG
      ...
      IF(TOT.GT.0)THEN
       ...
       IF(MAFLAG.EQ.-1 .OR. MAFLAG.EQ.-3)THEN
        ...
        WRITE(0,30)" MS: ",MSN,MSNN
        WRITE(0,20)"(MS is not an input parameter)"
       ELSEIF(MAFLAG.EQ.-2 .OR. MAFLAG.EQ.-4)THEN
        IF(MSFLAG.EQ.0)THEN
       ...
       IF(MAFLAG.EQ.-5)THEN
        ...
        WRITE(0,30)" XIF: ",XIFN,XIFNN
        WRITE(0,30)" XIS: ",XISN,XISNN
        WRITE(0,30)"MHD: ",MHDN,MHDNN
        WRITE(0,20)"(MHD is not an input parameter)"
        WRITE(0,30)"MHU: ",MHUN,MHUNN
        WRITE(0,20)"(MHU is not an input parameter)"
        WRITE(0,30)" MS: ",MSN,MSNN
        WRITE(0,20)"(MS is not an input parameter)"
       ELSE
        WRITE(0,30)"MUEFF: ",MUN,MUNN
        WRITE(0,20)"(MU is not an input parameter)"
        IF(MHDFLAG.EQ.1)WRITE(0,30)"MHD: ",MHDN,MHDNN
        IF(MHUFLAG.EQ.1)WRITE(0,30)"MHU: ",MHUN,MHUNN
       ENDIF

       IF(MUPN.NE.0d0 .OR. MUPNN.NE.0d0)
     . WRITE(0,30)" MUP: ",MUPN,MUPNN
       IF(MSPN.NE.0d0 .OR. MSPNN.NE.0d0)
     . WRITE(0,30)" MSP: ",MSPN,MSPNN
       IF(M3HN.NE.0d0 .OR. M3HNN.NE.0d0)
     . WRITE(0,30)" M3H: ",M3HN,M3HNN
