...
************************************************************************
     ...
     . AU3IN,AU3INN,AD3IN,AD3INN,AE3IN,AE3INN,
     . LIN,LINN,KIN,KINN,XIFIN,XIFINN,XISIN,XISINN,
     . MUPIN,MUPINN,MSPIN,MSPINN,M3HIN,M3HINN
...
*   Moduli and phases
      ...
      IF(REALP(3).NE.0d0)THEN
       ...
       MYPHASES(3)=ATAN(IMAGP(3)/REALP(3))
      ELSE
       PAR(20)=IMAGP(3)
      ...
      IF(REALP(4).NE.0d0)THEN
       ...
       MYPHASES(4)=ATAN(IMAGP(4)/REALP(4))
      ELSE
       PAR(21)=IMAGP(4)
      ...
      IF(REALP(5).NE.0d0)THEN
       ...
       MYPHASES(5)=ATAN(IMAGP(5)/REALP(5))
      ELSE
       PAR(22)=IMAGP(5)
      ...
      IF(REALP(6).NE.0d0)THEN
       ...
       MYPHASES(6)=ATAN(IMAGP(6)/REALP(6))
      ELSE
       PAR(12)=IMAGP(6)
      ...
      IF(REALP(7).NE.0d0)THEN
       ...
       MYPHASES(7)=ATAN(IMAGP(7)/REALP(7))
      ELSE
       PAR(13)=IMAGP(7)
      ...
      IF(REALP(8).NE.0d0)THEN
       ...
       MYPHASES(8)=ATAN(IMAGP(8)/REALP(8))
      ELSE
       PAR(14)=IMAGP(8)
      ...
      IF(REALP(11).NE.0d0)THEN
       ...
       MYPHASES(15)=ATAN(IMAGP(11)/REALP(11))
      ELSE
       MUP=IMAGP(11)
      ...
      IF(REALP(12).NE.0d0)THEN
       ...
       MYPHASES(13)=ATAN(IMAGP(12)/REALP(12))
      ELSE
       MSP=IMAGP(12)
      ...
      IF(REALP(13).NE.0d0)THEN
       ...
       MYPHASES(16)=ATAN(IMAGP(13)/REALP(13))
      ELSE
       M3H=IMAGP(13)
      ...
      IF(REALP(9).NE.0d0)THEN
       ...
       MYPHASES(14)=ATAN(IMAGP(9)/REALP(9))
      ELSE
       XIF=IMAGP(9)
      ...
      IF(REALP(10).NE.0d0)THEN
       ...
       MYPHASES(12)=ATAN(IMAGP(10)/REALP(10))
      ELSE
       XIS=IMAGP(10)
...
*   Check for errors
 ...
 29   IF(GMUFLAG.LT.0 .OR. GMUFLAG.GT.1)THEN
      ...
      IF(LMIN.EQ.1d99)THEN
       WRITE(0,1)"RE(LAMBDA)_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      ...
      IF(MUMIN.EQ.1d99)THEN
       WRITE(0,1)"RE(MUEFF)_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF

      IF(M1MIN.EQ.1d99 .AND. M1MAX.NE.1d99)THEN
       WRITE(0,1)"RE(M1)_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF

      IF(M2MIN.EQ.1d99)THEN
       WRITE(0,1)"RE(M2)_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF

      IF(M3MIN.EQ.1d99 .AND. M3MAX.NE.1d99)THEN
       WRITE(0,1)"RE(M3)_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
      ...
      IF(ALMIN.EQ.1d99 .AND. ALMAX.NE.1d99)THEN
       WRITE(0,1)"RE(AL)_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF

      IF(AKMIN.EQ.1d99 .AND. AKMAX.NE.1d99)THEN
       WRITE(0,1)"RE(AK)_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF

      IF(XIFMIN.EQ.1d99 .AND. XIFMAX.NE.1d99)THEN
       WRITE(0,1)"RE(XIF)_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF

      IF(XISMIN.EQ.1d99 .AND. XISMAX.NE.1d99)THEN
       WRITE(0,1)"RE(XIS)_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF

      IF(XISIMIN.EQ.1d99 .AND. XISIMAX.NE.1d99)THEN
       WRITE(0,1)"IM(XIS)_min MUST BE GIVEN IN BLOCK EXTPAR"
       ERR=1
      ENDIF
...
*   Relations between (RE(ALAMBDA), MA, RE(XIF)) and (RE(AKAPPA), MP, RE(XIS))
     ...
     . " CAN BE GIVEN IN BLOCK EXTPAR"
      ...
      IF((KIMIN.EQ.0d0 .OR. KIMAX.EQ.0d0 .OR.
     . (KIMAX.NE.1d99 .AND. KIMIN*KIMAX.LT.0d0))
     . .AND. (KMIN.EQ.0d0 .OR. KMAX.EQ.0d0 .OR.
     ...
     .   " CAN BE GIVEN IN BLOCK EXTPAR"
       ...
       IF(XISIMIN.NE.1d99)THEN
        WRITE(0,1)"IF KAPPA CAN BE 0, IM(XIS) IS NOT AN INPUT"
        ERR=1
       ENDIF
       IF(NXIFI.NE.0)THEN
        WRITE(0,1)"NXIFI CANNOT BE GIVEN IN BLOCK STEPS"
        ERR=1
       ELSE
        NXIFI=1
       ENDIF
      ELSE
       IF(NXIFI.EQ.0)NXIFI=1
      ENDIF
...
*   Number of points
     ...
     . NMSP.LE.0 .OR. NM3H.LE.0 .OR. NAQ.EQ.0 .OR. NMQ.EQ.0 .OR.
...
*   Warnings
      ...
      IF(MUPMIN.EQ.MUPMAX .AND. NMUP.GT.1)THEN
       WRITE(0,1)"WARNING MUP_min = MUP_max => NMUP=1"
       NMUP=1
      ENDIF
      IF(MSPMIN.EQ.MSPMAX .AND. NMSP.GT.1)THEN
       WRITE(0,1)"WARNING MSP_min = MSP_max => NMSP=1"
       NMSP=1
      ENDIF
      IF(M3HMIN.EQ.M3HMAX .AND. NM3H.GT.1)THEN
       WRITE(0,1)"WARNING M3H_min = M3H_max => N3H=1"
       NM3H=1
      ENDIF
      IF(AQMIN.EQ.AQMAX .AND. NAQ.GT.1)THEN
       WRITE(0,1)"WARNING AU3_min = AU3_max => NAU3=1"
       NAQ=1
      ENDIF
      IF(MQMIN.EQ.MQMAX .AND. NMQ.GT.1)THEN
       WRITE(0,1)"WARNING MQ3_min = MQ3_max => NMQ3=1"
       NMQ=1
      ENDIF
      IF(LIMIN.EQ.LIMAX .AND. NLI.GT.1)THEN
       WRITE(0,1)"WARNING IM(LAMBDA)_min = IM(LAMBDA)_max => NLI=1"
       NLI=1
      ENDIF
      IF(KIMIN.EQ.KIMAX .AND. NKI.GT.1)THEN
       WRITE(0,1)"WARNING IM(KAPPA)_min = IM(KAPPA)_max => NKI=1"
       NKI=1
      ENDIF
      IF(M1IMIN.EQ.M1IMAX .AND. NM1I.GT.1)THEN
       WRITE(0,1)"WARNING IM(M1)_min = IM(M1)_max => NM1I=1"
       NM1I=1
      ENDIF
      IF(M2IMIN.EQ.M2IMAX .AND. NM2I.GT.1)THEN
       WRITE(0,1)"WARNING IM(M2)_min = IM(M2)_max => NM2I=1"
       NM2I=1
      ENDIF
      IF(M3IMIN.EQ.M3IMAX .AND. NM3I.GT.1)THEN
       WRITE(0,1)"WARNING IM(M3)_min = IM(M3)_max => NM3I=1"
       NM3I=1
      ENDIF
      IF(AU3IMIN.EQ.AU3IMAX .AND. NAU3I.GT.1)THEN
       WRITE(0,1)"WARNING IM(AU3)_min = IM(AU3)_max => NAU3I=1"
       NAU3I=1
      ENDIF
      IF(AD3IMIN.EQ.AD3IMAX .AND. NAD3I.GT.1)THEN
       WRITE(0,1)"WARNING IM(AD3)_min = IM(AD3)_max => NAD3I=1"
       NAD3I=1
      ENDIF
      IF(AE3IMIN.EQ.AE3IMAX .AND. NAE3I.GT.1)THEN
       WRITE(0,1)"WARNING IM(AE3)_min = IM(AE3)_max => NAE3I=1"
       NAE3I=1
      ENDIF
      IF(XIFIMIN.EQ.XIFIMAX .AND. NXIFI.GT.1)THEN
       WRITE(0,1)"WARNING IM(XIF)_min = IM(XIF)_max => NXIFI=1"
       NXIFI=1
      ENDIF
      IF(XISIMIN.EQ.XISIMAX .AND. NXISI.GT.1)THEN
       WRITE(0,1)"WARNING IM(XIS)_min = IM(XIS)_max => NXISI=1"
       NXISI=1
      ENDIF
      IF(MUPIMIN.EQ.MUPIMAX .AND. NMUPI.GT.1)THEN
       WRITE(0,1)"WARNING IM(MUP)_min = IM(MUP)_max => NMUPI=1"
       NMUPI=1
      ENDIF
      IF(MSPIMIN.EQ.MSPIMAX .AND. NMSPI.GT.1)THEN
       WRITE(0,1)"WARNING IM(MSP)_min = IM(MSP)_max => NMSPI=1"
       NMSPI=1
      ENDIF
      IF(M3HIMIN.EQ.M3HIMAX .AND. NM3HI.GT.1)THEN
       WRITE(0,1)"WARNING IM(M3H)_min = IM(M3H)_max => NM3HI=1"
       NM3HI=1
      ENDIF
      ...
      IF(MUPMIN.NE.MUPMAX .AND. NMUP.EQ.1)THEN
       WRITE(0,10)"WARNING NMUP=1 => MUP_max=MUP_min=",MUPMIN
       MUPMAX=MUPMIN
      ENDIF
      IF(MSPMIN.NE.MSPMAX .AND. NMSP.EQ.1)THEN
       WRITE(0,10)"WARNING NMSP=1 =>  MSP_max= MSP_min=",MSPMIN
       MSPMAX=MSPMIN
      ENDIF
      IF(M3HMIN.NE.M3HMAX .AND. NM3H.EQ.1)THEN
       WRITE(0,10)"WARNING NM3H=1 => M3H_max=M3H_min=",M3HMIN
       M3HMAX=M3HMIN
      ENDIF
      IF(AQMIN.NE.AQMAX .AND. NAQ.EQ.1)THEN
       WRITE(0,10)"WARNING NAU3=1 => AU3_max=AU3_min=",AQMIN
       AQMAX=AQMIN
      ENDIF
      IF(MQMIN.NE.MQMAX .AND. NMQ.EQ.1)THEN
       WRITE(0,10)"WARNING NMQ3=1 => MQ3_max=MQ3_min=",MQMIN
       MQMAX=MQMIN
      ENDIF
      IF(LIMIN.NE.LIMAX .AND. NLI.EQ.1)THEN
       WRITE(0,10)"WARNING NLI=1 => IM(LAMBDA)_max=IM(LAMBDA)_min=",
     . LIMIN
       LIMAX=LIMIN
      ENDIF
      IF(KIMIN.NE.KIMAX .AND. NKI.EQ.1)THEN
       WRITE(0,10)"WARNING NKI=1 => IM(KAPPA)_max=IM(KAPPA)_min=",KIMIN
       KIMAX=KIMIN
      ENDIF
      IF(M1IMIN.NE.M1IMAX .AND. NM1I.EQ.1)THEN
       WRITE(0,10)"WARNING NM1I=1 => IM(M1)_max=IM(M1)_min=",M1IMIN
       M1IMAX=M1IMIN
      ENDIF
      IF(M2IMIN.NE.M2IMAX .AND. NM2I.EQ.1)THEN
       WRITE(0,10)"WARNING NM2I=1 => IM(M2)_max=IM(M2)_min=",M2IMIN
       M2IMAX=M2IMIN
      ENDIF
      IF(M3IMIN.NE.M3IMAX .AND. NM3I.EQ.1)THEN
       WRITE(0,10)"WARNING NM3I=1 => IM(M3)_max=IM(M3)_min=",M3IMIN
       M3IMAX=M3IMIN
      ENDIF
      IF(AU3IMIN.NE.AU3IMAX .AND. NAU3I.EQ.1)THEN
       WRITE(0,10)"WARNING NAU3I=1 => IM(AU3)_max=IM(AU3)_min=",AU3IMIN
       AU3IMAX=AU3IMIN
      ENDIF
      IF(AD3IMIN.NE.AD3IMAX .AND. NAD3I.EQ.1)THEN
       WRITE(0,10)"WARNING NAD3I=1 => IM(AD3)_max=IM(AD3)_min=",AD3IMIN
       AD3IMAX=AD3IMIN
      ENDIF
      IF(AE3IMIN.NE.AE3IMAX .AND. NAE3I.EQ.1)THEN
       WRITE(0,10)"WARNING NAE3I=1 => IM(AE3)_max=IM(AE3)_min=",AE3IMIN
       AE3IMAX=AE3IMIN
      ENDIF
      IF(XIFIMIN.NE.XIFIMAX .AND. NXIFI.EQ.1)THEN
       WRITE(0,10)"WARNING NXIFI=1 => IM(XIF)_max=IM(XIF)_min=",XIFIMIN
       XIFIMAX=XIFIMIN
      ENDIF
      IF(XISIMIN.NE.XISIMAX .AND. NXISI.EQ.1)THEN
       WRITE(0,10)"WARNING NXISI=1 => IM(XIS)_max=IM(XIS)_min=",XISIMIN
       XISIMAX=XISIMIN
      ENDIF
      IF(MUPIMIN.NE.MUPIMAX .AND. NMUPI.EQ.1)THEN
       WRITE(0,10)"WARNING NMUPI=1 => IM(MUP)_max=IM(MUP)_min=",MUPIMIN
       MUPIMAX=MUPIMIN
      ENDIF
      IF(MSPIMIN.NE.MSPIMAX .AND. NMSPI.EQ.1)THEN
       WRITE(0,10)"WARNING NMSPI=1 => IM(MSP)_max=IM(MSP)_min=",MSPIMIN
       MSPIMAX=MSPIMIN
      ENDIF
      IF(M3HIMIN.NE.M3HIMAX .AND. NM3HI.EQ.1)THEN
       WRITE(0,10)"WARNING NM3HI=1 => IM(M3H)_max=IM(M3H)_min=",M3HIMIN
       M3HIMAX=M3HIMIN
      ENDIF
...
*********************************************************************
     ...
     . MSFLAG,AKFLAG,ALFLAG
      ...
      DO I=1,7
       S=S+NFAIL(I)
      ENDDO
      WRITE(0,20)"  with mh1^2 or mhc^2 < 0               ",S
     ...
     .  WRITE(0,30)" RE(XIF): ",XIFN,XIFNN,DEV(XIFN,XIFNN)
       ELSEIF(MOD(MAFLAG,3).EQ.1)THEN
        ...
        WRITE(0,20)"(RE(ALAMBDA) is not an input parameter)"
     ...
     .  WRITE(0,30)" RE(XIF): ",XIFN,XIFNN,DEV(MAN,MANN)
       ELSE
        ...
        WRITE(0,20)"(RE(XIF) is not an input parameter)"
       ENDIF
       IF(MAFLAG/3.EQ.0)THEN
     ...
     .  WRITE(0,30)" RE(XIS): ",XISN,XISNN,DEV(XISN,XISNN)
       ELSEIF(MAFLAG/3.EQ.1)THEN
        ...
        WRITE(0,20)"(RE(AKAPPA) is not an input parameter)"
     ...
     .  WRITE(0,30)" RE(XIS): ",XISN,XISNN,DEV(XISN,XISNN)
       ELSE
        ...
        WRITE(0,20)"(RE(XIS) is not an input parameter)"
       ENDIF
     ...
     . WRITE(0,30)" IM(XIF): ",XIFIN,XIFINN,DEV(XIFIN,XIFINN)
       IF(XISIN.NE.0d0 .OR. XISINN.NE.0d0)
