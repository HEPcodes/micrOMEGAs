...
\noindent
$\bullet$ \verb|pvalIC22( nu_flux, nuB_flux,&L)|\\
 calculates the p-value for  given $\nu_{\mu}$ and  $\bar{\nu}_{\mu}$  fluxes, ~\cite{Belanger:2015hra}. The fluxes are assumed to be in
$[\rm{GeV\;km}^2\;\rm{Year}]^{-1}$.  This function uses
the \verb|IC22BGdCos(cs)| and \verb|IC22sigma(E)| angular distribution for background and signal as well as the event files distributed  by IceCube22 
%, events file
%presented by IceCube22 and channel distribution for background and signal.
with  $\phi<\phi_{cut}=8^{\circ} $.  The returned parameter \verb|L|  corresponds to  $\log({\cal L}(0,\phi_{cut})/{\cal L}(1,\phi_{cut}))$, 
with $\cal L$ defined in ~\cite{Belanger:2015hra}.\\
\noindent  
$\bullet$ \verb|fluxFactorIC22(pval, nu,nuBar)|\\
For given neutrino, \verb|nu|, and anti-neutrino fluxes, \verb|nuBar|, this function returns the factor that should be applied to the fluxes (neutralino-proton   cross sections)  to obtain a given p-value {\tt pval} in IceCube22. This is used to obtain  limits on the SD cross section for given neutrino fluxes.
